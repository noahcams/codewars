...
object EqualSidesOfAnArray {
    ...
    /** Returns the first index that has a sum to the left and right that are equal, or -1 if there isn't one */
    fun findEvenIndex(arr:IntArray):Int {
        arr.forEachIndexed { i, _ ->
            val previousSum = if (i == 0) 0 else arr.slice(0 until i).sum()
            val postSum = arr.slice((i + 1) until arr.size).sum()
            if (previousSum == postSum) return i
        }
        return -1
    }
}
...
fun main() {
    println(EqualSidesOfAnArray.findEvenIndex(intArrayOf(1, 3, 2)))
}
